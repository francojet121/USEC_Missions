...
//Initate Debarkation Control
	...
	[_listBox] call fnc_usec_lha_UpdateListBox;
...
while {LHAAlive} do {
	...
	//Update Bay Status
	{
		...
		_NearObjectsLand = ( nearestObjects [ _CheckPos, ['Land','USEC_CargoContainer1','USEC_CargoContainer2','Misc_cargo_cont_tiny'],_radius]);
		...
		if (count _allObjects > 0) then {
			if (_bayStatus) then {
				[_bay,false] call fnc_usec_lha_BayStatusUpdate;	
			};
