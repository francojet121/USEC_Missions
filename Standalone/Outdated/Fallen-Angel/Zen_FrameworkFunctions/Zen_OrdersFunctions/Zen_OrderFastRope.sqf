...
_testCan = [((_heliPosition vectorAdd _offsetL) vectorAdd [0,0,-(_offsetL select 2)]), "Land_Can_Dented_F", ((_heliPosition select 2) - 2)] call Zen_SpawnVehicle;
waitUntil {
    sleep 0.1;
    ((((velocity _testCan) select 2) > -0.05) || (ZEN_STD_OBJ_ASLPositionZ(_testCan) < 0.25))
};

_height = ZEN_STD_OBJ_ATLPositionZ(_testCan);
