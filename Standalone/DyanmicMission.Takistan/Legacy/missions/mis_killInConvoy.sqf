...
//TARGET
	...
	targetAway = false;
	USEC_MissionCleanup set [count USEC_MissionCleanup, _target];
...
//PUBLISH MISSION
	USEC_MissionPublisher = [2,MissionNumber,_target];
...
//START	
	...
	if (!targetAway) then {
		...
		[] spawn {
			while {true} do {
				hintSilent format ["%1",fearValue];
			};
		};
		
		while {(alive _target) and (canMove _target) and !targetAway} do {
				...
				if (({canMove _x} count _vehicleList) > 0) then {
					{
						...
						if ((canMove _x) and (_canDrive > 0)) then {
							...
							[_target] orderGetIn true;
							waitUntil{!(canMove _x) or !(alive _target) or targetAway};
						};
