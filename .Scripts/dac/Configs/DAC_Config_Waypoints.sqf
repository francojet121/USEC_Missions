...
if((_DAC_WP_Typ < 5) && (_DAC_WP_Typ >= 0)) then
{
	switch (_DAC_WP_Nr) do
	{
		case 0:	{
					...
					_CheckRadius1 	= [	10,		10,		10,		20,		20		];          // Critical Radius - No objects may exist
					_CheckRadius2 	= [	15,		20,		20,		40,		40		];          // Uncritical Radius - Objects of certain size/height may exists
					_checkAreaH   	= [	40,		15,		20,		10,		20		];          // Maximum Altitude change in Uncritical Radius
					_checkMaxH    	= [	5000,	5000,	5000,	5000,	5000	];          // Maximum Terrain Height
					_checkMinH    	= [	1,		1,		1,		1,		3		];            // Minimum Terrain Height
					_checkNear    	= [	20,		30,		30,		100,	300		];          // Minimum distance between waypoints
					_checkObjH1    	= [	1.5,	0.5,	0.5,	0.2,	2		];            // Maximum object size for Critical Area
					_checkObjH2    	= [	30,		15,		15,		5,		30		];          // Maximum object size for Uncritical Area
					_checkCount   	= [	200,	200,	200,	500,	1500	];          // Number of attempts to find a waypoint
					_checkResol   	= [	45,		36,		36,		12,		12		];          // Value for the raster-scan resolution at the height values
    ...
    case 6:	{ // Tight Cities
          //----------------- #Sol----#Veh----#Tan----#Air----#Camp----;
					_CheckRadius1 = [	0,		  0,		  0,		  0,		  0		];          // Critical Radius - No objects may exist
					_CheckRadius2 = [	0,		  0,		  0,		  0,		  0		];          // Uncritical Radius - Objects of certain size/height may exists
					_checkAreaH   = [	20,		  15,		  20,		  10,		  20		];          // Maximum Altitude change in Uncritical Radius
					_checkMaxH    = [	5000,	  5000,	  5000,	  5000,	  5000	];          // Maximum Terrain Height
					_checkMinH    = [	0,		  0,		  0,		  0,		  0		];            // Minimum Terrain Height
					_checkNear    = [	50,		  50,		  50,		  50, 	  50		];          // Minimum distance between waypoints
					_checkObjH1   = [	2,	    2,	    2,	    2, 	    2		];            // Maximum object size for Critical Area
					_checkObjH2   = [	50,		  50,		  50,		  50,		  50		];          // Maximum object size for Uncritical Area
					_checkCount   = [	200,	  200,	  200,	  200,	  200	];          // Number of attempts to find a waypoint
					_checkResol   = [	1,		  1,		  1,		  12,	  	12		];          // Value for the raster-scan resolution at the height values
