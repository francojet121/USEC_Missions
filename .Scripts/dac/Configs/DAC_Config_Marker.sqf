...
//-----------------------------------------------------------------------------------------------
	case 1:	// Dev-Knyte
	{		
				_setShowZones 		= 2;  // 0/1/2 Off/During Init/On
				_setShowWPs 		    = 2;  // 0/1/2 Off/During Init/On
				_setShowUnit 		  = [1,1,1,1];  // 0/1 Off/On [West,East,Indep,Civ]
				_setGroupType 		= 0;  // 0/1 PerGroup/PerUnit
				_setMarkerText		= ["if(format[""%1"",_group] == ""<NULL-group>"") then {format[""<--- %1"",typeof _unit]} else {format[""%1"",count units _group]}"];
				_setMarkerDel 		= 1;  // 0/1/2 Do not delete/Delete if group killed/Delete if group deleted
				_setMarkerRefresh = [0.3,0.3];  // Time between refresh [unit marker,other marker]
				_setSizeWpLine 		= [1,1];  // 0/#, 0 disable, [line thickness, waypoint size]
				_setSizeLeaderLine= 1;  // 0/#, 0 disable
				_setSizeZoneLine 	= 2;  // 0/#, 0 disable
				_setSizeCampLine 	= 4;  // 0/#, 0 disable
				_setSizeZoneBorder= 4;  // 0/#, 0 disable
				_setArtiMarker 		= 1;  // 0/1 Off/On
				_setCampMarker 		= 2;  // 0/1/2 Off/Camp Objects Only/All Objects
				_setSideColor 		= [   // [East, West, Indep, Civ, Neutral, Waypoint Lines, Waypoints]
										"ColorRed",
										"ColorBlue",
										"ColorYellow",
										"ColorGreen",
