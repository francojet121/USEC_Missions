...
if(isNil "DAC_Radio_Dist")	then {	DAC_Radio_Dist    = "((500 * DAC_AI_Level)"  };  // "((1000 * DAC_AI_Level) * _skillgroup)"						};
if(isNil "DAC_Alert_Dist")	then {	DAC_Alert_Dist    = "(400 + (250 * DAC_AI_Level))"  };// "(100 + (150 * DAC_AI_Level))"								};
if(isNil "DAC_Radio_Time")	then {	DAC_Radio_Time    = "10"    }; // "((10 / DAC_AI_Level) / ((skill (leader _group)) + 0.1))"	};
