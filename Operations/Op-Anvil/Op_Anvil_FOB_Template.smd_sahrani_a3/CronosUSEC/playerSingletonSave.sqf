private ["_type", "_playerObject", "_id", "_isLoaded", "_isOnDisconnect", "_shouldSave", "_name", "_playerPos", "_playerDir", "_playerHeadgear", "_playerGoggles", "_uniform", "_uniformCargo", "_uniformMagazines", "_uniformWeapons", "_vestCargo", "_vestMagazines", "_vestWeapons", "_vest", "_backpack", "_backpackCargo", "_backpackMagazines", "_backpackWeapons", "_inventory", "_weapons", "_primaryWepAttachments", "_secondaryWepAttachments", "_handgunAttachments", "_weaponMagazines", "_inVehicle", "_alive", "_selectedWeapon", "_currentStance", "_fatigue", "_serverReply"];
...
//_serverReply = ["argumentTest", "Type is ", _type ] call invoke_java_method;
...
if (_type == "validate" && _serverReply == "success") then{
  ...
  ["infantryLoaded", _id ] call invoke_java_method;
  (owner _playerObject) publicVariableClient "loadingDone";
};


if (_serverReply == "failed") then{
  ["errorReport", "Failed to validate player equipment", "playerLoading", _id, _name ] call invoke_java_method;
  (owner _playerObject) publicVariableClient "terminatePlayer";
};

if (_serverReply == "terminated") then{
  (owner _playerObject) publicVariableClient "terminatePlayer";
};
//if _serverReply is kick then kick player
