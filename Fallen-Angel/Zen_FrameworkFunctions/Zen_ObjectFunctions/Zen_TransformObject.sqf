...
private ["_obj", "_pos", "_heightChange", "_speed", "_dir", "_isNormal", "_setNormal"];

if !([_this, [["OBJECT"], ["VOID"], ["SCALAR"], ["SCALAR"], ["SCALAR"], ["BOOL"]], [], 2] call Zen_CheckArguments) exitWith {
...
_heightChange = 0;
_speed = vectorMagnitude [(velocity _obj) select 0, (velocity _obj) select 1, 0];
_dir = getDir _obj;
_isNormal = false;
...
if (local _obj) then {
    0 = [_obj, _speed, _dir, _setNormal] spawn Zen_TransformObject_Orient_MP;
} else {
    Zen_MP_Closure_Packet = ["Zen_TransformObject_Orient_MP", [_obj, _speed, _dir, _setNormal]];
