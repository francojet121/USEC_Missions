...
if ((local _unit) && {(_unit in _heli)}) then {
    ...
    0 = [_unit, _unit, -2, 0, (getDir _unit)] call Zen_TransformObject;
    _ropePoint = 22;
    ...
    waitUntil {
        ...
        _nearBuilding = false;
        _unitHeight = ZEN_STD_OBJ_ATLPositionZ(_unit);
        ...
        sleep 0.25;
        ((_unitHeight < 3) || (ZEN_STD_OBJ_ASLPositionZ(_unit) < 3) || _nearBuilding || ((_heli distance _unit) > 51))
    };
    ...
    waitUntil {
        sleep 0.5;
        ((isTouchingGround _unit) || ZEN_STD_OBJ_ASLPositionZ(_unit) < 1)
    };
};
