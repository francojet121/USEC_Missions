...
switch (toLower _objType) do {
    ...
    case "convoy": {
        ...
        0 = [_objects, _objPos] spawn {
            private ["_troopVeh", "_vehicles", "_leadVeh", "_vehicle", "_endPoint", "_troopUnits", "_patrolGroup"];

            _vehicles = _this select 0;
            _endPoint = _this select 1;
            _leadVeh = (_this select 0) select 0;
            _troopVeh = (_this select 0) select 2;

            waitUntil {
                sleep 2;
                ((({!(canMove _x) || !(alive _x)} count _vehicles) > 0) || (((_troopVeh distance _endPoint) < 75) && (speed _troopVeh < 1)))
            };
